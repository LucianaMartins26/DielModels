...
def pca(df_sampling_path, df_kstest_path):
    ...
    for i in range(len(pca_result)):
        for j in range(i + 1, len(pca_result)):
            if np.linalg.norm(pca_result[i] - pca_result[j]) < overlap_threshold:
    ...
    overlapping_reactions_no_reverse = [tuplo for tuplo in overlapping_reactions if
                                        ("reverse" not in tuplo[0] and "reverse" not in tuplo[1])]
    overlapping_reactions_day_night = [tuplo for tuplo in overlapping_reactions_no_reverse if
                                       ("Day" in tuplo[0] and "Night" in tuplo[1]) or (
                                                   "Night" in tuplo[0] and "Day" in tuplo[1])]
